...
void main() {
  ...
  group('SupabaseRemoteStore helpers', () {
    ...
    test(
      'buildUpsertRows falls back to defaultScope when callback returns null',
      () {
        final items = <_Rec>[_Rec('a', 'A', DateTime.utc(2023, 1, 1))];
        final cfg = store.config.copyWith(scopeForUpsert: (_) => null);
        final s2Store = SupabaseRemoteStore<_Rec, String>(config: cfg);
        final rows = s2Store.buildUpsertRows(items);
    ...
    test(
      'groupDeletesByScope groups ids using scopeForDelete then defaultScope',
      () {
        const s2 = SyncScope('records', {'userId': 'u2'});
        final cfg = store.config.copyWith(
          scopeForDelete: (id) => id == 'x'
              ? s2
              : null, // x uses s2, others fallback to defaultScope
        );
        final s2Store = SupabaseRemoteStore<_Rec, String>(config: cfg);
