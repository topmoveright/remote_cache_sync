...
void main() {
  ...
  group('AppwriteRemoteStore fetchSince integration (raw pages)', () {
    ...
    test('buildFetchQueries builds scope + since filters', () {
      ...
      expect(queries.any((q) => q.contains('scope_name') && q.contains('records')), isTrue);
      expect(queries.any((q) => q.contains('updated_at') && q.contains(since.toIso8601String())), isTrue);
    });
    ...
    test('parsePage handles same-timestamp mixed rows', () {
      final ts = DateTime.utc(2024, 2, 1, 12);
      final rows = <Map<String, dynamic>>[
        {
          'id': 'x1',
          'title': 'X1',
    ...
    test('parsePage treats explicit null deleted_at as upsert', () {
      final ts = DateTime.utc(2024, 3, 1);
      final rows = <Map<String, dynamic>>[
        {
          'id': 'n1',
          'title': 'N1',
