...
void main() {
  group('SimpleSyncOrchestrator.readWith', () {
    ...
    test('offlineOnly evaluates only local cache', () async {
      ...
      final items = await orch.readWith(scope, spec, policy: CachePolicy.offlineOnly);
      expect(items.map((e) => e.id), containsAll(['L1', 'L2']));
      expect(items.any((e) => e.id == 'R1'), isFalse);
    });

    test('localFirst returns local immediately and triggers background sync', () async {
      ...
      await remote.batchUpsert([R('R1', 'remote', base.add(const Duration(minutes: 1))) ]);

      final spec = QuerySpec(
        filters: [FilterOp(field: 'updatedAt', op: FilterOperator.gte, value: base)],
      );

      final first = await orch.readWith(scope, spec, policy: CachePolicy.localFirst);
    ...
    test('remoteFirst syncs then evaluates locally', () async {
      ...
      final items = await orch.readWith(scope, spec, policy: CachePolicy.remoteFirst);
      expect(items.length, 1);
      expect(items.first.id, 'R2');
    });

    test('preferRemoteEval upserts remote-evaluated results before local evaluation', () async {
